...
from tiagen import (  # noqa: E402
    emit_hmi, emit_scl, emit_seq, emit_tags, import_steps, lint, model, sequence,
    validate,
)
...
class TestLint(unittest.TestCase):
    """`lint` checks a project that already exists, including what nobody generated."""

    LIBRARY = os.path.join(REPO_ROOT, "library", "scl")

    def _lint(self, files):
        """Write files into a temp dir and lint it."""
        root = tempfile.mkdtemp(prefix="tiagen-lint-")
        for name, body in files.items():
            with open(os.path.join(root, name), "w", encoding="utf-8") as fh:
                fh.write(body)
        rules = lint.load_rules()
        project = lint.load_project(root, library_dir=self.LIBRARY)
        return lint.run(project, rules), project, rules

    def _ids(self, findings):
        return {f.rule_id for f in findings}

    def test_generated_output_is_clean(self):
        # The strongest test there is: the generator's own output must satisfy the
        # house rules. If it does not, one of the two is wrong.
        for example in ("minimal.yaml", "conveyor-line.yaml", "step-sequence.yaml"):
            spec = model.load(os.path.join(REPO_ROOT, "spec", "examples", example))
            out = tempfile.mkdtemp(prefix="tiagen-lintbuild-")
            build_mod.build(spec, out_dir=out)
            rules = lint.load_rules()
            project = lint.load_project(os.path.join(out, "scl"), library_dir=self.LIBRARY)
            findings = lint.run(project, rules)
            self.assertEqual(
                [], findings,
                msg=f"{example}: " + "; ".join(f"{f.rule_id} {f.message}" for f in findings),
            )

    def test_a_modified_library_block_is_an_error(self):
        with open(os.path.join(self.LIBRARY, "20_FB_Motor.scl"), encoding="utf-8") as fh:
            original = fh.read()
        tampered = original.replace("#FbOnTimer(IN :=", "#FbOnTimer(IN := FALSE AND", 1)
        self.assertNotEqual(original, tampered, "the tamper target moved; update this test")

        findings, _, _ = self._lint({"20_FB_Motor.scl": tampered})
        self.assertIn("L001", self._ids(findings))

    def test_reformatting_a_library_block_is_not_reported_as_drift(self):
        with open(os.path.join(self.LIBRARY, "20_FB_Motor.scl"), encoding="utf-8") as fh:
            original = fh.read()
        # Extra blank lines, changed indentation and a translated comment must not
        # register as a functional change, or the rule becomes noise and gets disabled.
        reformatted = original.replace("\n    ", "\n        ")
        reformatted = reformatted.replace("// running feedback supervision time",
                                          "// Ueberwachungszeit der Rueckmeldung")
        findings, _, _ = self._lint({"20_FB_Motor.scl": reformatted})
        self.assertNotIn("L001", self._ids(findings))

    def test_hand_edited_generated_block_is_an_error(self):
        body = (
            "// Generated by tiagen - do not edit.\n"
            'FUNCTION_BLOCK "FB_Thing"\n'
            "   VAR_INPUT\n      A : Bool;\n   END_VAR\n"
            "BEGIN\n"
            "    // TODO put this back after the trial\n"
            "    #A := FALSE;\n"
            "END_FUNCTION_BLOCK\n"
        )
        findings, _, _ = self._lint({"gen.scl": body})
        self.assertIn("L002", self._ids(findings))

    def test_duplicate_output_write_and_direct_output_are_reported(self):
        body = (
            "// A hand-written block that drives outputs itself.\n"
            'FUNCTION_BLOCK "FB_Hand"\n'
            "   VAR_INPUT\n      A : Bool;\n   END_VAR\n"
            "BEGIN\n"
            '    %Q0.0 := #A;\n'
            '    %Q0.0 := NOT #A;\n'
        ...
        findings, _, _ = self._lint({"hand.scl": body})
        found = self._ids(findings)
        self.assertIn("L005", found)      # written twice
        self.assertIn("L006", found)      # written outside a device FB
        self.assertIn("L007", found)      # absolute address

    def test_bypass_identifiers_are_flagged_but_prose_is_not(self):
        body = (
            "// A block whose comments mention overriding, which is documentation.\n"
            'FUNCTION_BLOCK "FB_Words"\n'
            "   VAR\n      BypassInterlock : Bool;\n   END_VAR\n"
            "   VAR_TEMP\n      vScratch : Bool;\n   END_VAR\n"
            "BEGIN\n"
            "    // an assignment here overrides the active step's request\n"
            "    #vScratch := #BypassInterlock;\n"
            "END_FUNCTION_BLOCK\n"
        )
        findings, _, _ = self._lint({"words.scl": body})
        messages = " ".join(f.message for f in findings if f.rule_id == "L003")
        self.assertIn("BypassInterlock", messages)
        # VAR_TEMP is a keyword and "overrides" here is prose - neither is a bypass.
        self.assertNotIn("VAR_TEMP", messages)
        self.assertNotIn("overrides", messages)

    def test_a_block_documented_after_its_closer_counts_as_documented(self):
        body = (
            'TYPE "UDT_Thing"\n   STRUCT\n      A : Bool;\n   END_STRUCT;\n\nEND_TYPE\n'
            "\n(*\n   What this type is for.\n*)\n"
        )
        findings, _, _ = self._lint({"udt.scl": body})
        self.assertNotIn("L009", self._ids(findings))

    def test_utf16_export_is_read_rather_than_seen_as_empty(self):
        root = tempfile.mkdtemp(prefix="tiagen-lint-utf16-")
        body = (
            "// A block.\n"
            'FUNCTION_BLOCK "FB_Wide"\n   VAR\n      BypassThing : Bool;\n   END_VAR\n'
            "BEGIN\n    ;\nEND_FUNCTION_BLOCK\n"
        )
        with open(os.path.join(root, "wide.scl"), "w", encoding="utf-16") as fh:
            fh.write(body)
        project = lint.load_project(root, library_dir=self.LIBRARY)
        self.assertEqual(1, len(project.blocks))
        self.assertEqual("FB_Wide", project.blocks[0].name)

    def test_unparsed_formats_are_reported_as_unchecked_not_clean(self):
        findings, project, rules = self._lint({"Block.s7dcl": "BLOCK Whatever\nsome text\n"})
        self.assertEqual(1, len(project.blocks))
        self.assertFalse(project.blocks[0].structured)
        report = lint.format_report(findings, project, rules)
        self.assertIn("read as text only", report)

    def test_every_enabled_rule_is_either_implemented_or_named_as_missing(self):
        rules = lint.load_rules()
        self.assertTrue(rules, "rules.yaml produced no rules")
        missing = [r.id for r in rules if r.enabled and not r.implemented]
        # Not an assertion that nothing is missing - an assertion that the tool says
        # so out loud, because a silently skipped rule reads as a passing one.
        report = lint.format_report([], lint.Project(), rules)

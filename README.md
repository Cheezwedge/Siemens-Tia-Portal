...
| | |
|---|---|
| **Generator** | Tested. 90 unit tests over addressing, naming, validation, SCL emission, tag XML well-formedness, and both example specs end to end. `python -m unittest discover -s generator/tests` |
| **SCL library** | **Compiled clean in TIA Portal V21** (`V21.00.01.00_00.04.00.01`) on an S7-1200 G2. All 15 generated sources imported as external sources - 0 errors, 0 warnings - and the project compiles clean including the generated step sequencer. Imported by hand rather than through the driver, so this verifies the SCL, not the driver. |
